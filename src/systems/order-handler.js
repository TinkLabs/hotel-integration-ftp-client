...
import { reorderKnex, ENUM_REORDER_STATUS } from '../database/sqlite';
...
async function receiveChunk(raw) {
  ...
  let row = await reorderKnex('reorder_messages').where('id', uniqueId).first('*');
  if (row === undefined) {
    reorderKnex('reorder_messages').insert({
      id: uniqueId,
    ...
    reorderKnex('reorder_messages').where('id', uniqueId).update({ status: ENUM_REORDER_STATUS.REORDER_RECEIVING });
  ...
  await reorderKnex('reorder_message_chunks').insert({
    id: chunkId, message_id: uniqueId, sequence: chunkSeq, content: raw,
  });

  let sql = 'UPDATE reorder_messages SET chunk_received_count = chunk_received_count + 1, status = CASE chunk_received_count WHEN chunk_count THEN ? ELSE status END WHERE id = ?';
  await reorderKnex.raw(sql, [ENUM_REORDER_STATUS.REORDER_RECEIVED, uniqueId]);
}

async function reorderChunk() {
  // send to hai queue one by one;
  let message = await reorderKnex('reorder_messages').where('status', ENUM_REORDER_STATUS.REORDER_RECEIVED).first('*');
  if (message === undefined) {
    return;
  }

  let messageId = message.id;
  await reorderKnex('reorder_messages').where('id', messageId).update({ status: ENUM_REORDER_STATUS.REORDER_SENDING });
  try {
    for (let i = 0; i < message.chunk_count; i += 1) {
      // eslint-disable-next-line no-await-in-loop
      let chunk = await reorderKnex('reorder_message_chunks').where({ message_id: messageId, sequence: i }).first('*');
      // eslint-disable-next-line no-await-in-loop
      await sendToHaiQueue(chunk.content);
    }

    await reorderKnex('reorder_messages').where('id', messageId).update({ status: ENUM_REORDER_STATUS.REORDER_FINISH });
    await reorderKnex('reorder_message_chunks').where({ message_id: messageId }).delete();
    ...
    await reorderKnex('reorder_messages').where('id', messageId).update({ status: ENUM_REORDER_STATUS.REORDER_ERROR });
  }
}
